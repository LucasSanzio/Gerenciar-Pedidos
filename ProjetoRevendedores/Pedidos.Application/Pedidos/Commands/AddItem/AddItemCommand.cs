...
/// </summary>
public record AddItemCommand(Guid PedidoId, Guid ProdutoId, string NomeProduto, int PrecoCentavos) : IRequest<PedidoDto>;
...
public class AddItemCommandHandler : IRequestHandler<AddItemCommand, PedidoDto>
{
    ...
    public async Task<PedidoDto> Handle(AddItemCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            pedido.AdicionarItem(request.ProdutoId, request.NomeProduto, request.PrecoCentavos);
        }
